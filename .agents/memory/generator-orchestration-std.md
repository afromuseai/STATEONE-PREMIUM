...
- **Stable idea ref pattern (BI → Chatbot parity)**: The controller calls `bridge.getCurrentIdea()` immediately after registration — before the typewriter has written to React state. Every module that drives `getCurrentIdea()` from React state MUST maintain a parallel stable ref (e.g. `chatbotIdeaRef`, `marcusBiIdeaRef`) and set it at ALL idea ingress points: (1) `onPopulate` in `useGeneratorOrchestration` (covers pending intent + workspace signals), and (2) `bridge.populate()` (covers bridge-direct calls). Setting it only in the bridge misses the hook's populate path.
