...
// Bridges the Website Studio session state onto the independent `generationBus`
// that `GenerationActivity` renders from.
//
// This file is backend-agnostic. It only accepts a small, plain snapshot shape
// (`WebsiteGenerationSnapshot`) and turns *changes* to that snapshot into
// `GenerationEvent`s on the bus.
//
// The caller (a component that already has legitimate access to session state
// — e.g. `StudioShell`, which receives `session` as a prop) is responsible for
// extracting the snapshot and calling `useWebsiteGenerationAdapter(snapshot)`
// every render. That keeps this adapter reusable and decoupled.
//
// Nothing here modifies AgentConversation, the generation event bus/types, or
// GenerationActivity — it only calls `generationBus.emit(...)`.
...
// ─── Snapshot shape — deliberately generic, not backend-typed ────────────────
...
// ─── React hook — call this every render from a component that has session
// state, passing the extracted snapshot. Safe to call with the same (or idle)
// snapshot repeatedly; only transitions cause bus emissions. ─────────────────
