...
import type { WSSessionState } from "@/lib/website-studio-session/types"
...
// Bridges the Website Studio session state onto the generation event bus, so
// `GenerationActivity` (rendered inside AgentConversation) reflects real builds.
// The adapter itself is backend-agnostic — it only receives a plain snapshot
// from `session` and translates changes to bus events.
...
export type SideView = "chat" | "explorer" | "collaboration" | null
...
interface StudioShellProps {
  project:    V2Project
  onRefresh:  () => void
  /** Optional Website Studio session — when provided (during generation), the side panel
   *  shows the live streaming activity instead of the editing chat. */
  session?:   WSSessionState | null
...
export function StudioShell({ project, onRefresh, session, previewGenerating }: StudioShellProps) {
  ...
  // Auto-open chat panel when generation is in progress
  useEffect(() => {
  ...
  // Feed the session snapshot into the generation event bus so the inline
  // `GenerationActivity` (inside AgentConversation) reflects the real build.
  // Safe to call with session === null — the adapter treats that as idle.
  ...
  const [chatInput,      setChatInput]      = useState<string | null>(null) // P2
  ...
  // ── Direct persistence — every file change (from agent or manual editing) is
  // written straight to the project, applied immediately with no separate
  // confirmation/accept step. Preview is regenerated once generation finishes.
  ...
  // ── P2: Route inline AI commands to the chat panel ────────────────────────────
  const handleInlineCommand = useCallback((prompt: string) => {
    setSideView("chat")
    setChatInput(prompt)
  }, [])

  // ── P1: Ask AI via command palette ────────────────────────────────────────────
  ...
  // ── Context snapshot for agent (Phase 4) ─────────────────────────────────
    ...
    >
      ...
      <div className="flex min-h-0 w-full flex-1 overflow-hidden">
        <StudioSidebar>
          ...
          {/* Side panel — Chat, Explorer, or Collaboration */}
