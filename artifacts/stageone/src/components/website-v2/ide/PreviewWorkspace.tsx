import { useState } from "react"
import { Monitor, Smartphone, Tablet, RefreshCw, ExternalLink, Wifi } from "lucide-react"

type ViewportSize = "desktop" | "tablet" | "mobile"

const VIEWPORTS: Record<ViewportSize, { label: string; icon: React.ElementType; width: string }> = {
  desktop: { label: "Desktop", icon: Monitor,    width: "100%" },
  tablet:  { label: "Tablet",  icon: Tablet,     width: "768px" },
  mobile:  { label: "Mobile",  icon: Smartphone, width: "390px" },
}

interface PreviewWorkspaceProps {
  preview: string | null
  projectName: string
}

export function PreviewWorkspace({ preview, projectName }: PreviewWorkspaceProps) {
  const [viewport, setViewport] = useState<ViewportSize>("desktop")
  const [refreshKey, setRefreshKey] = useState(0)

  const vp = VIEWPORTS[viewport]

  return (
    <div className="flex h-full flex-col bg-[#0c0c0c]">
      {/* Preview toolbar */}
      <div className="flex flex-shrink-0 items-center gap-2 border-b border-white/[0.07] bg-[#0d0d0d] px-3 py-1.5">
        {/* Browser chrome dots */}
        <div className="flex items-center gap-1.5">
          <div className="h-2.5 w-2.5 rounded-full bg-red-500/50" />
          <div className="h-2.5 w-2.5 rounded-full bg-yellow-500/50" />
          <div className="h-2.5 w-2.5 rounded-full bg-emerald-500/50" />
        </div>

        {/* URL bar */}
        <div className="flex flex-1 items-center gap-2 rounded-md border border-white/[0.07] bg-white/[0.03] px-3 py-1 text-[11px] text-white/30">
          <Wifi className="h-3 w-3 text-emerald-400/60" />
          <span className="truncate">https://website-studio.stageone.dev</span>
          <span className="ml-auto flex-shrink-0 rounded-full bg-emerald-400/15 px-1.5 py-0.5 text-[10px] font-semibold text-emerald-400">
            Live
          </span>
        </div>

        {/* Viewport + refresh */}
        <div className="flex items-center gap-0.5">
          {(Object.keys(VIEWPORTS) as ViewportSize[]).map((v) => {
            const Icon = VIEWPORTS[v].icon
            const active = viewport === v
            return (
              <button
                key={v}
                onClick={() => setViewport(v)}
                title={VIEWPORTS[v].label}
                className={`flex h-6 w-6 items-center justify-center rounded transition-colors
                  ${active ? "bg-amber-400/15 text-amber-400" : "text-white/30 hover:bg-white/5 hover:text-white/60"}`}
              >
                <Icon className="h-3.5 w-3.5" />
              </button>
            )
          })}
          <div className="mx-1 h-3.5 w-px bg-white/10" />
          <button
            onClick={() => setRefreshKey((k) => k + 1)}
            title="Refresh"
            className="flex h-6 w-6 items-center justify-center rounded text-white/30 transition-colors hover:bg-white/5 hover:text-white/60"
          >
            <RefreshCw className="h-3.5 w-3.5" />
          </button>
          <button
            title="Open in new tab"
            className="flex h-6 w-6 items-center justify-center rounded text-white/30 transition-colors hover:bg-white/5 hover:text-white/60"
          >
            <ExternalLink className="h-3.5 w-3.5" />
          </button>
        </div>
      </div>

      {/* Preview content */}
      <div className="flex flex-1 items-start justify-center overflow-auto bg-zinc-950/60 p-4">
        {!preview ? (
          <div className="flex h-full w-full items-center justify-center">
            <div className="text-center">
              <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl border border-white/[0.07] bg-white/[0.02]">
                <Monitor className="h-8 w-8 text-white/20" />
              </div>
              <p className="text-sm font-semibold text-white/30">No preview yet</p>
              <p className="mt-1 text-xs text-white/20">Ask the AI agent to build your website</p>
            </div>
          </div>
        ) : (
          <div
            className="relative overflow-hidden rounded-xl border border-white/[0.08] shadow-2xl transition-all duration-300"
            style={{ width: vp.width, maxWidth: "100%", minHeight: "480px" }}
          >
            <iframe
              key={refreshKey}
              srcDoc={preview}
              title={`Preview — ${projectName}`}
              sandbox="allow-scripts allow-same-origin"
              className="block w-full border-0 bg-white"
              style={{ height: "calc(100vh - 200px)", minHeight: "540px" }}
            />
          </div>
        )}
      </div>
    </div>
  )
}
