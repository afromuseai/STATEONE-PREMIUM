...
export function CommandPalette({
  open, onClose, activeFile, onAskAI, onDeploy, onCodeReview,
}: CommandPaletteProps) {
  ...
  const COMMANDS: PaletteCommand[] = [
    {
      id: "ask-ai-builder", label: "Ask AI Builder", category: "AI",
    ...
    },
    {
      ...
      description: "Generate a new React component with AI",
      icon: Code2, color: "#818cf8",
      action: () => { onAskAI("Create a new reusable React component for this project. Ask me what it should do."); onClose() },
    },
    {
      ...
      action: () => {
        ...
        onAskAI(`Explain what ${file} does. Walk me through the logic step by step.`)
        onClose()
      },
    },
    {
      ...
      action: () => {
        const file = activeFile ? activeFile.split("/").pop() : "the active file"
        onAskAI(`Refactor ${file} for better readability and performance. Follow the existing code style.`)
        onClose()
      },
    },
    {
      id: "generate-api", label: "Generate API endpoint", category: "AI",
      description: "Create a new API route with AI",
      icon: Globe, color: "#60a5fa",
      action: () => { onAskAI("Generate a new API endpoint. What should it do?"); onClose() },
    },
    {
      ...
      action: () => { onAskAI("Install a new package. Which one should I add?"); onClose() },
    },
    {
      ...
      action: () => {
        const file = activeFile ? activeFile.split("/").pop() : "the active file"
        onAskAI(`Generate comprehensive unit tests for ${file}. Cover edge cases.`)
        onClose()
      },
    },
    {
