...
export function WebContainerProvider({ project, children }: Props) {
  ...
  // Track synced files for the agent-edit diff (N5)
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      if (pipelineComplete && wcSingleton) {
        ...
        // Re-populate syncedFilesRef so agent-sync can diff correctly
  ...
  // ── N5: Sync agent edits → WC FS on project.files change ────────────────────
  useEffect(() => {
    const wc = wcSingleton
    if (!wc || status !== "ready") return
    ...
    for (const file of project.files) {
      ...
      wc.fs.writeFile(normalized, file.content).catch((err) => {
        console.error("[WC] Agent sync writeFile failed:", file.path, err)
      })
