...
router.post(
  ...
  async (req, res): Promise<void> => {
    ...
    // All phase starts, completions, file events, and narration messages are now
    // generated from real bus events instead of manual engine.emit* calls in the route.
    const engineBusUnsub = engine.attachTaskBus(bus);

    bus.emit("pipeline", "start", "running", { userId }, "pipeline");
    // ↑ engine subscriber: emitPhaseStart("UNDERSTAND") — buffered, flushed below.
    ...
    try {
      ...
      // ↑ SSE subscriber: sseWrite({ phase: "start", model, industry })
      // ↑ Engine subscriber: "I'm designing the website architecture..." — flush immediately.
      flushEngine(engine, res);
      ...
      // Flush UNDERSTAND complete — events generated by validation:schema completed bus emit above.
      // Engine subscriber: emitPhaseComplete("UNDERSTAND") + "Preparing the implementation..."
      flushEngine(engine, res);
      ...
      // ↑ Engine subscriber: emitPhaseStart("DESIGN") — flush immediately.
      flushEngine(engine, res);
      ...
      // Design review finished (success or best-effort) — close DESIGN phase via bus.
      bus.emit("llm", "architect_complete", "completed", {
        durationMs: Date.now() - reviewStart,
      }, "design-review");
      // ↑ Engine subscriber: emitPhaseComplete("DESIGN")
      flushEngine(engine, res);
      ...
      // Flush BUILD phase start — events generated by llm:codegen_start bus emit above.
      // Engine subscriber: emitPhaseStart("BUILD")
      flushEngine(engine, res);
      ...
      try {
        ...
        // Emit filesystem events for every LLM-generated file.
        // Engine subscriber translates each into a FileActivityEvent (no manual calls needed).
        ...
        if (!existingPaths.has("package.json")) {
          ...
          // ↑ Engine subscriber: emitWarning + emitFileOperation for injected file
          flushEngine(engine, res);
        ...
        // codegen_complete only buffered the message; emitPhaseComplete("BUILD") is
        // called here explicitly, after infra injections, so BUILD closes with the
        // full file set.
      ...
      if (projectId) {
        ...
        // Flush REPORT phase start — events generated by database:save_files running bus emit above.
        // Engine subscriber: emitPhaseStart("REPORT") + "Saving your project..."
        flushEngine(engine, res);
        ...
        // No project to persist — open REPORT phase directly so the phase
        // lifecycle is coherent when pipeline:finish closes it below.
      ...
      // Flush REPORT complete + Done — events generated by pipeline:finish bus emit above.
      // Engine subscriber: emitPhaseComplete("REPORT") + emitDone()
      flushEngine(engine, res);
      ...
      // Prevents memory leaks and cross-request event delivery.
      engineBusUnsub();
      unsubscribeBus();
      bus.clear();
    }
